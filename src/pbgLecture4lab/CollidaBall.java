...
public interface CollidaBall {
    ...
    static boolean collidesWith(CollidaBall a, CollidaBall b, double delta){//};//{
        ...
        if (a.getVectorTo(b).mag() < a.getRadius() + b.getRadius()){
    ...
    /**
     * obtains the vector distance between this collidaball and the other one
     * @param other the other collidaball
     * @return the distance between this one and the other one
     */
    public default Vect2D getVectorTo(CollidaBall other){
        return Vect2D.minus(other.getPos(), getPos());
    }
    ...
    /**
     * Works out whether or not this particle is inactive
     * @return true if inacctive, false if active.
     */
    boolean isInactive();
    ...
    static double getExactCollisionTime(CollidaBall a, CollidaBall b){
        ...
        final Vect2D c = a.getVectorTo(b);
